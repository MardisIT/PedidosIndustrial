...
daoCodigosNuevos=dataManager.getDaoCodigosNuevos();
	...
	private void verificarYPedirPermisosDeCamara() {
		int estadoDePermiso = ContextCompat.checkSelfPermission(a.getApplicationContext(), Manifest.permission.CAMERA);
	...
	public boolean validaCodigoArticulo() {
		...
		articulo = daoArticulo.getByKey(valor.toString().trim());
		if (articulo==null){
	...
	public boolean validaCantidadIntroducidaConStock(){
		try{
			...
			if(unidadmedida.toString().trim().equals("Cajas")) {
				pedidoItem.setCantidad(cantidadingresada);
				total = Utilidades.Redondear(((pedidoItem.getCantidad() * articulo.getPrecio2()) * preciodelproducto) - totaldescuentoprontopago - valornegociacionespecial - valornegociacionespecialavena, 2);
			}else{
				pedidoItem.setCantidad(Integer.valueOf(valor));
...
public  double CalcularDescuentoMix(String codigoProductoActual,String opcion,double cantidad,double precio){
	...
	public void eliminarPedido(boolean eliminarAutomaticamente,long _idPedidoAEliminar) {
	...
	public void scanArticulo() 
	{
		...
		else
	    {
			if (!permisoCamaraConcedido) {
				Toast.makeText(a.getApplicationContext(), "Por favor permite que la app acceda a la cámara", Toast.LENGTH_SHORT).show();
